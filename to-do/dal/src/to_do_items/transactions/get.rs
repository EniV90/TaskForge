...
pub type GetAllResponse = Result<Vec<ToDoItem>, SchedulerServiceError>;
...
async fn sqlx_postgres_get_all(user_id: i32) -> GetAllResponse {
    let items = sqlx::query_as::<_, ToDoItem>(
        "
        SELECT * FROM to_do_items WHERE id IN ( SELECT to_do_id FROM user_connections WHERE user_id = $1 )",
    ).bind(user_id)
